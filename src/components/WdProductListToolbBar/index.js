		...
		return (
			<View style={styles.toolbarContainer}>
				...
				<View
					style={[
						styles.toolbarRight,
						!showSorting && { justifyContent: 'flex-end' },
					]}>
					{showSorting && (
						<TouchableOpacity
					...
					<TouchableOpacity
						style={{ width: 20 }}
						onPress={() => this.layoutChangeHandler()}>
						<Icon
							style={styles.toolbarIcon}
...
function mergeProps(stateProps, dispatchProps, ownProps) {
	...
	return {
		...
		sortByRating: (per_page, page, order, order_by) => {
			return actions.sortByRating(dispatch, per_page, page, order, order_by);
		},
		sortByDate: (per_page, page, order, order_by) => {
			return actions.sortByDate(dispatch, per_page, page, order, order_by);
		},
		sortByPriceDesc: (per_page, page, order, order_by) => {
			return actions.sortByPriceDesc(dispatch, per_page, page, order, order_by);
		},
		sortByPriceAsc: (per_page, page, order, order_by) => {
			return actions.sortByPriceAsc(dispatch, per_page, page, order, order_by);
		},
	};
}
