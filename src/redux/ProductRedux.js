...
export const actions = {
	...
	fetchRelatedProducts: async (dispatch, productID) => {
		...
		if (json === undefined) {
			dispatch({
				type: types.FETCH_RELATED_PRODUCTS_FAIL,
				message: Languages.ErrorMessageRequest,
			});
		} else if (json.code) {
			dispatch({
...
export const reducer = (state = initialState, action) => {
	...
	switch (type) {
		...
		case types.FETCH_RELATED_PRODUCTS_PENDING:
		case types.FETCH_NEW_ARRIVALS_PENDING: {
			return {
				...state,
				isFetching: true,
				error: null,
		...
		case types.FETCH_PRODUCTS_SUCCESS: {
			return Object.assign({}, state, {
				isFetching: false,
				list: state.list.concat(items), // needed for showing individual category product
				listAll: state.listAll.concat(items),
				stillFetch: items.length !== 0,
				error: null,
				productFinish: finish,
